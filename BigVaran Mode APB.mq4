...
int iAPBRedBlueCandle(BuffersiAPB &Buff, bool Option)
  {
   if(Option == IS_BUFFER)
     {
      if(Buff.CloseBuffer[1] < Buff.OpenBuffer[1])
        {
         return BLUE_CANDLE;
        }
      else
      ...
      if(Option == IS_TWO_CANDLE)
        {
         if(Buff.CloseBuffer[0] < Buff.OpenBuffer[0])
           {
            return BLUE_CANDLE;
           }
         else
...
void CheckTouchLineiTMA(double &CopyBuffLowerBand[], double &CopyBuffUpperBand[], double &CopyBuffCenterLine[])
  ...
  */
void CheckTouchLineiTMA(double &CopyBuffLowerBand[], double &CopyBuffUpperBand[], double &CopyBuffCenterLine[], iAPB &BuffCandle)
  {
//Red APB
   if(BuffCandle.GetCloseBuffer(1) >= CopyBuffUpperBand[1] && BuffCandle.GetOpenBuffer(1) <= CopyBuffUpperBand[1])
     {
      TouchLine.TouchUpperBand = true;
     }
   else
      if(BuffCandle.GetHighBuffer(1) > CopyBuffUpperBand[1] && BuffCandle.GetOpenBuffer(1) <= CopyBuffUpperBand[1])
        {
         TouchLine.TouchUpperBand = true;
        }
      else
         if(BuffCandle.GetHighBuffer(1) > CopyBuffUpperBand[1] && BuffCandle.GetOpenBuffer(1) >= CopyBuffUpperBand[1] && BuffCandle.GetCloseBuffer(1) <= CopyBuffUpperBand[1])
           {
            TouchLine.TouchUpperBand = true;
           }
         // Blue APB
         else
            if(BuffCandle.GetOpenBuffer(1) >= CopyBuffUpperBand[1] && BuffCandle.GetCloseBuffer(1) <= CopyBuffUpperBand[1])
              {
               TouchLine.TouchUpperBand = true;
              }
            else
               if(BuffCandle.GetHighBuffer(1) > CopyBuffUpperBand[1] && BuffCandle.GetCloseBuffer(1) <= CopyBuffUpperBand[1])
                 {
                  TouchLine.TouchUpperBand = true;
                 }
               else
                  if(BuffCandle.GetHighBuffer(1) > CopyBuffUpperBand[1] && BuffCandle.GetCloseBuffer(1) >= CopyBuffUpperBand[1] && BuffCandle.GetOpenBuffer(1) <= CopyBuffUpperBand[1])
                    {
                     TouchLine.TouchUpperBand = true;
                    }

//Blue APB
   if(BuffCandle.GetCloseBuffer(1) <= CopyBuffLowerBand[1] && BuffCandle.GetOpenBuffer(1) >= CopyBuffLowerBand[1])
     {
      TouchLine.TouchLowerBand = true;
     }
   else
      if(BuffCandle.GetLowBuffer(1) < CopyBuffLowerBand[1] && BuffCandle.GetCloseBuffer(1) >= CopyBuffLowerBand[1])
        {
         TouchLine.TouchLowerBand = true;
        }
      else
         if(BuffCandle.GetLowBuffer(1) < CopyBuffLowerBand[1] && BuffCandle.GetOpenBuffer(1) >= CopyBuffLowerBand[1])
           {
            TouchLine.TouchLowerBand = true;
           }
         //Red APB
         else
            if(BuffCandle.GetOpenBuffer(1) >= CopyBuffLowerBand[1] && BuffCandle.GetCloseBuffer(1) <= CopyBuffLowerBand[1])
              {
               TouchLine.TouchLowerBand = true;
              }
            else
               if(BuffCandle.GetLowBuffer(1) < CopyBuffLowerBand[1] && BuffCandle.GetCloseBuffer(1) >= CopyBuffLowerBand[1])
                 {
                  ...
                  if(BuffCandle.GetLowBuffer(1) < CopyBuffLowerBand[1] && BuffCandle.GetCloseBuffer(1) <= CopyBuffLowerBand[1] && BuffCandle.GetOpenBuffer(1) >= CopyBuffLowerBand[1])
                    {
                     TouchLine.TouchLowerBand = true;
                    }


   if(BuffCandle.GetCloseBuffer(1) > CopyBuffCenterLine[1] && TouchLine.TouchLowerBand == true)
      TouchLine.TouchLowerBand = false;
   else
      if(BuffCandle.GetCloseBuffer(1) < CopyBuffCenterLine[1] && TouchLine.TouchUpperBand == true)
         TouchLine.TouchUpperBand = false;
   /*if(IsOrderClose(Ticket1) == false)
     {
      if(OrderSelect(Ticket1, SELECT_BY_TICKET)==true)
...
//|                                                                  |
//+------------------------------------------------------------------+
double HighLowCandleStopLoss(double LimitPrice, bool Trend, iAPB &BuffCandle)
  {
   double CurrentPrice = 0;
   if(Trend == HIGH_CANDLE)
      for(int i = 1; i < BuffCandle.GetBuffSize()-1; i++)
        {
         if(BuffCandle.GetCloseBuffer(i) > BuffCandle.GetCloseBuffer(i-1))
           {
            CurrentPrice = BuffCandle.GetHighBuffer(i);
           }
         else
            if(BuffCandle.GetCloseBuffer(i) < BuffCandle.GetCloseBuffer(i-1) && CurrentPrice != 0)
              {
               for(int k = i; k < BuffCandle.GetBuffSize()-1; k++)
                 {
                  if(BuffCandle.GetCloseBuffer(k) < BuffCandle.GetCloseBuffer(k-1) && CurrentPrice >= BuffCandle.GetCloseBuffer(k))
                    {
                     return CurrentPrice;;
                    }
                  else
      ...
      if(Trend == LOW_CANDLE)
         for(int i = 1; i < BuffCandle.GetBuffSize()-1-1; i++)
           {
            if(BuffCandle.GetCloseBuffer(i) < BuffCandle.GetCloseBuffer(i-1))
              {
               CurrentPrice = BuffCandle.GetLowBuffer(i);
              }
            else
               if(BuffCandle.GetCloseBuffer(i) > BuffCandle.GetCloseBuffer(i-1) && CurrentPrice != 0)
                 {
                  for(int k = i; k < BuffCandle.GetBuffSize()-1-1; k++)
                    {
                     if(BuffCandle.GetCloseBuffer(k) > BuffCandle.GetCloseBuffer(k-1) && CurrentPrice <= BuffCandle.GetCloseBuffer(k))
                       {
                        return CurrentPrice;;
...
void StochLineCrossing()
  {
   ...
   if(OpenTicket == true || (LineTouch.TouchLine20 == LineTouch.TouchLine80 && LineTouch.TouchLine20 == true))
     {
      LineTouch.TouchLine20 = false;
      LineTouch.TouchLine80 = false;
      OpenTicket = false;
   ...
   if(Period() == PERIOD_M5)
     {
      ...
      CheckTouchLineiTMA(BuffTMA.lowerBand, BuffTMA.upperBand, BuffTMA.tma, APB);
      StochLineCrossing();
         ...
         if(OrderSelect(Ticket2, SELECT_BY_TICKET) == true)
           {
            if(OrderType() == OP_BUY)
              {
               if(APB.GetHighBuffer(0) >= iTMA.GetBuffUpperBand(1))
               ...
               if(OrderType() == OP_SELL)
                 {
                  if(APB.GetLowBuffer(0) <= iTMA.GetBuffLowerBand(1))
            ...
            if(OrderType() == OP_BUY)
              {
               ...
               if(NormalizeDouble(iTMA.GetBufftma(1), Digits) <= APB.GetLowBuffer(0) || NormalizeDouble(iTMA.GetBufftma(1), Digits) <= APB.GetOpenBuffer(0))
                 {
               ...
               if(OrderType() == OP_SELL)
                 {
                  if(NormalizeDouble(iTMA.GetBufftma(1), Digits) >= APB.GetHighBuffer(0) || NormalizeDouble(iTMA.GetBufftma(1), Digits) >= APB.GetOpenBuffer(0))
                    {
      ...
      //Print("LineTouch.TouchLine20 = ", LineTouch.TouchLine20);
      //Print("LineTouch.TouchLine80 = ", LineTouch.TouchLine80);
      if(Ticket1 == -1 && Ticket2 == -1 && TimeTrading(StartTimeTrading, EndTimeTrading) == true)
        {
         //BUY
         //Print("StartControlOpenOrder{");
         if((APB.GetCloseBuffer(1) > iTMA.GetBuffLowerBand(1) && APB.GetOpenBuffer(1) > iTMA.GetBuffLowerBand(1) && APB.GetLowBuffer(1) > iTMA.GetBuffLowerBand(1)
             && APB.GetCloseBuffer(1) > APB.GetCloseBuffer(2)) && iAPBRedBlueCandle(APB.GetBuffers(), IS_BUFFER) == BLUE_CANDLE && (TouchLine.TouchLowerBand == true) &&
            LineTouch.TouchLine20 == true && (StochasticK >= 20 || StochasticD >= 20))
           {
         ...
         //SELL
            ...
            if((APB.GetCloseBuffer(2) < iTMA.GetBuffUpperBand(2) && APB.GetOpenBuffer(2) < iTMA.GetBuffUpperBand(2) && APB.GetHighBuffer(2) <
                iTMA.GetBuffUpperBand(2) && APB.GetCloseBuffer(1) < APB.GetCloseBuffer(2)) && iAPBRedBlueCandle(APB.GetBuffers(), IS_BUFFER) == RED_CANDLE &&
